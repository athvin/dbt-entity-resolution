...
>
> Three things this file still records as open were *decided* by that reconciliation. **Stage 2b** closes as the explicit non-goal — v1 is full-rebuild, `is_incremental()` and the record-lifecycle machinery move to v2 together (§5 Stage 8). **Stage 4's** AC relaxes to *reachable* threshold constants. **`entity_clusters_1to1`** is tagged v2. **DR-17 and DR-16 have since closed too** (both CURRENT 2026-08-23): the model JSON is untrusted input validated once at compile time in the sidecar (`DesignDoc.md` §1.5), and the input contract is §2.0. **B.1 / DR-13 has since closed too** (CURRENT 2026-08-23): the harness reads only parquet and never opens the database; dbt keeps a file database. **DR-12 has since closed too** (CURRENT 2026-08-23): the label is `component_label`, it is not an identifier, and Stage 6b is an interface contract rather than a build (`DesignDoc.md` §1.6). **DR-08 and DR-09 have since closed too** (both CURRENT 2026-08-23, `DesignDoc.md` §1.7): the threshold is a dimension, and there are two of them with the gray band emitted to `review_pairs` rather than clustered. **B.8 is the only per-stage blocker left**, and it waits on the Stage 0.8 spike. Check `docs/backlog/LOOP-STATE.md` for live status before trusting any "blocked by" line here.
>
